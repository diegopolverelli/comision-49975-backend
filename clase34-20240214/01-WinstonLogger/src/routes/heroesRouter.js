...
router.post('/',(req,res)=>{
    let {name}=req.body
    if(!name){
        req.logger.log("silly", "Silly log - No se completo la propiedad name al dar de alta un heroe")
        req.logger.log("warn", "No se completo la propiedad name al dar de alta un heroe")
        req.logger.warn("No se completo la propiedad name al dar de alta un heroe")
        req.logger.error("error log - No se completo la propiedad name al dar de alta un heroe")
    ...
    if(!valido){
        req.logger.error({detalle:"error log - Se han ingresado propiedades invalidas", props:req.body})
        res.setHeader('Content-Type','application/json');
    ...
    if(existe){
        req.logger.error(`El heroe ${name} ya existe en DB`)
        res.setHeader('Content-Type','application/json');
    ...
    req.logger.info(`El heroe ${name} se ha generado correctamente`)
