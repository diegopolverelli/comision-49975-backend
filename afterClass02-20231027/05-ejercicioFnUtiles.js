/**
 * Dado un arreglo con personajes de Star Wars, crear una función
 * que reciba como argumento un arreglo, y un objero literal conteniendo 
 * características de los personajes, y permita filtrar en base a esto
 * el contenido del array.
 * Que la función busque coincidencia exacta entre las propiedades
 * indicadas en el objeto filtro
 * 
 * Realizar pruebas para verificar el funcionamiento. 
 * 
 * Modificar la función para que ahora tome coincidencias parciales
 * (que se pueda por ejemplo indicar nombre:"Sky" y la función de-
 * vuelva a Luke y a Anakin) 
 */
...
// filtro= {hair_color: "blond", skin_color: "fair"}
        
const filtrar=(arreglo=[], filtro={})=>{
    let propsFiltro=Object.keys(filtro)
    console.log(propsFiltro)
    return arreglo.filter(elementoArray=>{
        return propsFiltro.every(prop=>filtro[prop]===elementoArray[prop])
    })
}

let resultado=filtrar(starWars, {eye_color: "blue", skin_color: "fair"})
console.log(resultado)
resultado=filtrar(starWars, {name:"Sky"})
console.log(resultado)
